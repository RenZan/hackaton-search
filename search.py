...
def get_search_results(query, max_results=3):
    ...
    # Utilisation du paramètre 'time_range' pour limiter les résultats à l'année en cours (2025)
    params = {
        "q": query,
        "format": "json",
        "time_range": "year"
...
def extract_content(url, start_time=None, time_limit=None):
    ...
    try:
        response = session.post(CRAWL4AI_API_URL, json=payload, headers=headers, timeout=20)
        response.raise_for_status()
...
def analyze_chunk(chunk, query, initial_query, start_time=None, time_limit=None, max_retries=0):
    ...
    for attempt in range(max_retries + 1):
        if shutdown_flag or (start_time and time_limit and (time.time() - start_time > time_limit)):
            return []
        try:
            response = session.post(OPENAI_ENDPOINT, json=payload, headers=headers, timeout=30)
            response.raise_for_status()
            text = response.json().get("choices", [{}])[0].get("text", "").strip()
            logger.debug(f"[LLM] Réponse brute : {text}")
...
# Fonction pour générer de nouvelles requêtes avec le LLM
def generate_new_queries(current_knowledge, initial_query, start_time=None, time_limit=None, num_queries=3, max_retries=3):
    if shutdown_flag or (start_time and time_limit and (time.time() - start_time > time_limit)):
        return []
    logger.info("[LLM] Évaluation des learnings pour générer des requêtes pertinentes")
    prompt_eval = (
        ...
        f"jusqu'à 3 faits ou données qui méritent d'être approfondis (par ex., faits surprenants, spécifiques ou peu explorés), en tenant compte de leur pertinence à {CURRENT_DATE}. "
        f"Retourne chaque fait sous forme de liste à puces (exemple : '- Fait 1').\n\n"
        f"Informations disponibles :\n" + "\n".join([f"- {learning}" for learning in current_knowledge])
    )
    payload_eval = {"prompt": prompt_eval, "max_tokens": 1500, "temperature": 0.5}
    headers = {"Content-Type": "application/json"}
    try:
        response = session.post(OPENAI_ENDPOINT, json=payload_eval, headers=headers, timeout=30)
        response.raise_for_status()
        text = response.json().get("choices", [{}])[0].get("text", "").strip()
        key_learnings = re.findall(r"- (.+)", text)
        logger.info(f"[LLM] Learnings jugés intéressants : {key_learnings}")
    except requests.RequestException as e:
        logger.error(f"[LLM] Erreur lors de l'évaluation des learnings : {e}")
        key_learnings = current_knowledge[:3]

    prompt = (
        f"Tu es un expert en recherche. La date actuelle est {CURRENT_DATE}. En te basant sur les faits suivants liés à '{initial_query}', "
        f"génère {num_queries} questions spécifiques et approfondies pour creuser ces aspects, pertinentes à {CURRENT_DATE}. "
        f"Chaque question doit inclure au moins un terme spécifique (nom, date, lieu) tiré des faits et éviter les redondances.\n\n"
        f"Faits à approfondir :\n" + "\n".join([f"- {learning}" for learning in key_learnings]) + "\n\n"
        f"Retourne chaque question sous forme de liste à puces."
    )
    ...
    for attempt in range(max_retries + 1):
        ...
        try:
            ...
            if queries:
                logger.info(f"[LLM] Nouvelles requêtes générées : {queries}")
...
# Fonction principale avec recherche récursive
def deep_research(initial_query, breadth=3, depth=3, time_limit=180):
    ...
    prompt_initial = (
        f"Tu es un expert en recherche. La date actuelle est {CURRENT_DATE}. À partir du sujet suivant, formule une question de recherche précise et pertinente qui soit adaptée à des moteurs de recherche comme Google ou Bing par exemple, donc plutôt courte et directe/mot clés. :\n\n"
        f": '{initial_query}'.\n"
    ...
    headers = {"Content-Type": "application/json"}
    try:
        response = session.post(OPENAI_ENDPOINT, json=payload_initial, headers=headers, timeout=30)
        response.raise_for_status()
        ...
        queries_to_explore = [query]
    except Exception as e:
        logger.error(f"Erreur lors de la formulation de la première requête par le LLM : {e}")
    ...
    try:
        for iteration in range(depth):
            ...
            new_queries = generate_new_queries(all_learnings, initial_query, start_time=start_time, time_limit=time_limit)
            queries_to_explore.extend(new_queries)
            logger.info(f"Requêtes ajoutées à explorer : {new_queries}")
...
def generate_report(initial_query, all_learnings):
    ...
    prompt = (
        ...
        f"Ajoute une section finale qui synthétise et donne une perspective globale.\n\n"
...
if __name__ == "__main__":
    try:
        ...
        structured_data, report = deep_research(initial_query, breadth=5, depth=3, time_limit=180)
        print("\n### Données Structurées ###")
